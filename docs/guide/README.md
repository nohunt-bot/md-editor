...
| [`schema.md`](./schema.md) | MongoDB 資料模型（v2）：八個 collection（teams/skills/folders/skill_versions/tags/skill_likes/skill_presence/user_preferences）的欄位、index、規則、關聯圖與 migration 記錄，含「index 宣告 vs 實際建立」章節（原缺口已由 20260711 migration 收掉） | 要理解資料模型、或要改 schema/migration 的人 |
