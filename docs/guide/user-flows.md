...
```
[open 且 published？] ──否──▶ [不顯示複製鈕]
          │是
          ▼
[已是該團隊成員？] ──是──▶ [隱藏複製鈕（對你沒意義）]
          │否
          ▼
[有可編輯的目標團隊？] ──否──▶ [停用複製鈕＋提示「需要至少一個可編輯的團隊」]
          │是
          ▼
[恰好一個可編輯團隊？]
      ┌────是────┴────否────┐
      ▼                     ▼
[直接複製，導向新 skill]  [跳出選團隊 modal]
```
...
[每 5s 輪詢 presence]◀────────────────┐
  ...
  版本已變（別人存過檔）？ ──是──▶ [顯示「內容已被更新」軟提示]
   ...
   [儲存]
      │
      ▼
  版本不符？ ──是──▶ [409] → [ConflictDialog：覆蓋／合併／放棄]
      │否
      ▼
   [儲存成功]
```
